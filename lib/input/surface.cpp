...
Surface::Surface(std::string t_idfname, ControlType t_controltype) :
  idfname(std::move(t_idfname)),
  controltype(std::move(t_controltype))
{
}
...
std::vector<Surface> Surface::createSurfaces(const nlohmann::json & spawnjson, const nlohmann::json & jsonidf) {
  ...
  const auto frontback_control_surfaces = spawnjson.value("model",json()).value("buildingSurfaceDetailed", std::vector<json>(0));
  ...
  auto surface_control_type = [&](const std::string & idfname) {
    ...
    const auto frontbackit = std::find_if(std::begin(frontback_control_surfaces), std::end(frontback_control_surfaces), pred);
