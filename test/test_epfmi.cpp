...
// TODO: EnergyPlus 24.1 does not seem to balk at exceedingly large temperatures
// Find out if this is regression in Spawn or a bonified change in behavior for
// EnergyPlus.
//
// TLDR fix this test
...
//{
//  const auto fmu_file = create_epfmu();
//  spawn::fmu::FMU fmu{fmu_file, false, "invalid_input_test"};
//  CHECK(fmu.fmi.fmi2GetVersion() == std::string("2.0"));
//  const auto resource_path = (fmu.extractedFilesPath() / "resources").string();
//
//  fmi2CallbackFunctions callbacks = {
...
//  std::array<fmi2Real, 1> v{10000.0};
